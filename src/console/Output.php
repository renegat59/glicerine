...
class Output
{
    ...
    public static function writeError(string $text, int $color = Color::RED)
    {
        if ($color == null) {
            fwrite(STDERR, $text);
            return;
        }
        fwrite(STDERR, "\033[" . $color . "m" . $text . "\033[37m");
    }
    ...
    public function writeErrorLine(string $text, int $color = Color::RED)
    {
        self::writeError($text . PHP_EOL, $color);
    }
